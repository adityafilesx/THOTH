...
func managedParentPID(environment: [String: String]) -> pid_t? {
    guard let raw = environment["OmniMac_DESKTOP_PARENT_PID"],
...
let support = FileManager.default.homeDirectoryForCurrentUser
    .appendingPathComponent("Library/Application Support/OmniMac", isDirectory: true)
...
do {
    ...
    FileHandle.standardError.write(Data("OmniMac AX helper failed: \(error)\n".utf8))
    exit(EXIT_FAILURE)
}
