...
class MockSTTAdapter:
    ...
    def __init__(self, transcript: Transcript | None = None) -> None:
        self._transcript = transcript or Transcript(text="[mock transcription]", confidence=0.5, duration_s=0.0, language="en")
    ...
    async def transcribe(self, audio_bytes: bytes, mime: str) -> Transcript:
        try:
            ...
            raise STTUnavailableError(
                "faster-whisper is not installed; run `uv add faster-whisper` and download a model to enable local STT (pending live verification)"
            ) from exc
...
class WhisperCppSpeechRecognitionProvider:
    ...
    async def health(self) -> SpeechRecognitionHealth:
        ...
        return SpeechRecognitionHealth(
            ...
            model=str(self._model_path),
            loaded=self._loaded,
            detail=("local whisper.cpp runtime is ready; integrity verified" if self.integrity_pinned else "local whisper.cpp runtime is ready"),
        )
    ...
    async def transcribe(self, audio_bytes: bytes, mime: str) -> SpeechRecognitionResult:
        ...
        descriptor, raw_path = tempfile.mkstemp(prefix="omnimac-voice-", suffix=suffix)
        ...
        try:
            ...
            if return_code != 0:
                ...
                raise SpeechRecognitionUnavailable(f"local whisper.cpp transcription failed: {detail}")
            text = stdout.strip()
            if not text:
...
def _prepare_whisper_audio(audio: bytes, mime: str) -> tuple[bytes, str]:
    ...
    parameters = dict(part.split("=", 1) for part in parts[1:] if re.fullmatch(r"[a-z]+=[a-z0-9]+", part))
...
def default_stt_adapter() -> STTAdapter:
    ...
    selection = os.environ.get("OMNIMAC_STT", "whisper.cpp")
    if selection == "mock":
    ...
    provider = WhisperCppSpeechRecognitionProvider(
        executable=Path(os.environ.get("OMNIMAC_WHISPER_EXECUTABLE", "/opt/homebrew/bin/whisper-cli")),
        model_path=Path(
            os.environ.get(
                "OMNIMAC_WHISPER_MODEL_PATH",
                "data/models/whisper/ggml-base.en.bin",
            )
        ),
        language=os.environ.get("OMNIMAC_WHISPER_LANGUAGE", "en"),
        expected_executable_sha256=os.environ.get("OMNIMAC_WHISPER_EXECUTABLE_SHA256"),
        expected_model_sha256=os.environ.get("OMNIMAC_WHISPER_MODEL_SHA256"),
    )
