...
if TYPE_CHECKING:
    from omnimac_daemon.core.local_runtime import LocalAIRuntimeManager
...
class PiperSpeechSynthesisProvider:
    ...
    async def _render_and_play(self, request: SpeechRequest) -> int:
        descriptor, raw_path = tempfile.mkstemp(prefix="omnimac-piper-", suffix=".wav")
        ...
        try:
            ...
            return_code = await self._process.wait()
            self._state = SpeechPlaybackState.IDLE if return_code == 0 else SpeechPlaybackState.FAILED
            return return_code
        finally:
            self._process = None
...
class SpeechSynthesisService:
    ...
    async def cue(
        ...
        return await self._speak(SpeechRequest(segments=(segment,), voice=self._voice))
    ...
    async def _managed_playback(self, request: SpeechRequest) -> int:
        from omnimac_daemon.core.local_runtime import RuntimeComponent
