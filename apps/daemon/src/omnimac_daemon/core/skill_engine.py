    ...
    def expand(self, skill: SkillDefinition, inputs: dict[str, str], task_id: str) -> ExecutionPlan:
            ...
            if isinstance(value, str):
                for match in _PLACEHOLDER.finditer(value):
                    if match.group(1) not in inputs:
                        raise SkillInputError(f"unknown placeholder '{{{match.group(1)}}}' in skill '{skill.name}'")
        ...
        steps = [
            PlanStep(
                ...
                verification_checks=[VerificationCheck.model_validate(substitute(c.model_dump())) for c in step.verification_checks],
            )
...
def _builtin_skills() -> list[SkillDefinition]:
    ...
    return [
        ...
        SkillDefinition(
            ...
            steps=[
                ...
                SkillStep(
                    ...
                    declared_risk=RiskLevel.R1,
                    verification_checks=[VerificationCheck(kind=VerifierKind.FILE_EXISTS, params={"path": "{dest_path}"})],
                ),
            ],
        ),
        SkillDefinition(
        ...
        SkillDefinition(
            name="deep-research",
            description="Fetch a web page, summarize it using local AI, and save the result.",
            inputs=["url", "topic", "dest_path"],
            workflow=[
                "Fetch the web page",
                "Summarize its contents with respect to the topic using local inference",
                "Save the summarized text to the destination path",
            ],
            steps=[
                SkillStep(
                    title="Research {url} on '{topic}'",
                    tool_name="browser_research_and_save",
                    arguments={
                        "url": "{url}",
                        "topic": "{topic}",
                        "dest_path": "{dest_path}",
