...
# wrapped with the wake word "omnimac" and trivial politeness.
_BARE = [
    (ReflexKind.STOP, re.compile(r"^(omnimac[ ,]*)?stop( omnimac)?[.! ]*$")),
    (ReflexKind.CANCEL, re.compile(r"^(omnimac[ ,]*)?cancel( (the |this |that ?))?(task)?[.! ]*$")),
    (
        ReflexKind.STATUS,
    ...
    (ReflexKind.MUTE, re.compile(r"^(omnimac[ ,]*)?mute( omnimac| yourself)?[.! ]*$")),
    (
        ReflexKind.INTERRUPT,
        re.compile(r"^(omnimac[ ,]*)?(interrupt|be quiet|quiet|hush|stop speaking)( omnimac)?[.! ]*$"),
    ),
]
...
_APPROVAL_LANGUAGE = re.compile(r"^(please )?(yes|approve( it| the pending action)?|go ahead|do it)[.! ]*$")
...
class IntentRouter:
    ...
    def route(self, text: str) -> RoutedIntent:
        ...
        if m:
            ...
            if app := self._apps.get(arg):
                return RoutedIntent(tier=RouteTier.REFLEX, reflex_kind=ReflexKind.OPEN_APP, target=app, raw=text)
            clean_arg = re.sub(r"\s+in\s+(?:chrome|safari|browser)$", "", arg, flags=re.IGNORECASE)
            if url := _URL_ALIASES.get(clean_arg.lower()):
                return RoutedIntent(tier=RouteTier.REFLEX, reflex_kind=ReflexKind.OPEN_URL, target=url, raw=text)

        m = _FOCUS.match(command) or _BRING_FORWARD.match(command)
        if m and (app := self._apps.get(m.group("arg").strip())):
            return RoutedIntent(tier=RouteTier.REFLEX, reflex_kind=ReflexKind.FOCUS_APP, target=app, raw=text)

        m = _RUN.match(command)
        if m and (skill := self._resolve_skill(m.group("arg").strip())):
            return RoutedIntent(tier=RouteTier.REFLEX, reflex_kind=ReflexKind.RUN_SKILL, target=skill, raw=text)
...
def build_skill_aliases(skill_names: set[str]) -> dict[str, str]:
    ...
    aliases.update({phrase: target for phrase, target in _NATURAL_SKILL_ALIASES.items() if target in skill_names})
    return aliases
