...
class AXResolver:
    ...
    def resolve(
        ...
        *,
        ...
        app_elements = [element for element in elements if element.application_bundle_id == query.application_bundle_id]
        if elements and not app_elements:
            return self._reject("AX elements belong to a different application", freshness)
        scoped = [element for element in app_elements if query.window_identifier is None or element.window_identifier == query.window_identifier]
        eligible = [element for element in scoped if element.visible is not False and self._is_fresh(element, now)]
        ...
        if reference is not None:
            ...
            reference_matches = [element for element in eligible if element.reference_id == reference.reference_id]
        ...
        if query.identifier is not None:
            candidates = [element for element in eligible if element.identifier == query.identifier and self._role_matches(query, element)]
        ...
        if query.role is not None and query.label is not None:
            candidates = [element for element in eligible if self._role_matches(query, element) and element.label == query.label]
            result = self._candidate_result(
                candidates,
            ...
            candidates = [
                ...
                if self._role_matches(query, element) and element.label is not None and _normalize(element.label) == normalized_label
            ]
            result = self._candidate_result(
                candidates,
        ...
        if query.semantic_alias is not None:
            ...
            if alias is None:
                return self._reject("semantic alias is not declared by the trusted profile", freshness)
            if alias.application_bundle_id != query.application_bundle_id or alias.semantic_alias is not None:
                return self._reject("trusted profile alias is invalid for this application", freshness)
        ...
        if query.parent_path:
            candidates = [element for element in eligible if element.parent_path == query.parent_path and self._role_matches(query, element)]
            result = self._candidate_result(
                candidates,
        ...
        if query.role is not None and query.label is not None:
            fuzzy_candidates = [element for element in eligible if self._role_matches(query, element) and element.label is not None]
            if len(fuzzy_candidates) > self._max_fuzzy_candidates:
                return self._reject("too many candidates for bounded fuzzy resolution", freshness, re_resolved)
    ...
    def _fuzzy_result(
        ...
        ranked = sorted(
            ((SequenceMatcher(None, wanted, _normalize(element.label or "")).ratio(), element) for element in candidates),
...
def _potential_match(query: AXElementQuery, element: AXElementSnapshot) -> bool:
    ...
    if query.role is not None and query.label is not None:
        return element.role == query.role and _normalize(element.label or "") == _normalize(query.label)
    if query.parent_path:
