...
Per ADR-019: OmniMac's planner must NEVER execute tools (invariant 1), so it does
...
def build_system_prompt(registry: ToolRegistry) -> str:
    ...
    lines = [
        "You are OmniMac's planner. Produce a minimal JSON execution plan that a "
    ...
    for tool in registry.all():
        schema_json = tool.input_model.model_json_schema()
        args_str = str(schema_json.get("properties", {}))
        lines.append(
            f"- {tool.name} (default risk {tool.default_risk.value}; focus {tool.focus_policy.value}): {tool.description}. Arguments: {args_str}"
        )
    lines += [
        "",
        "Rules:",
