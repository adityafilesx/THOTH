...
from omnimac_daemon.schemas import ApprovalRequest, PlanStep, RiskLevel, StepStatus, Task, TaskState
...
def _user_safe_failure_reason(error: str) -> str:
    lowered = error.lower()
    if lowered.startswith("planning failed:") and any(marker in lowered for marker in _UNSAFE_PLAN_FAILURE_MARKERS):
...
class TaskPresentationComposer:
    ...
    def _fact(
        ...
        if state is TaskState.WAITING_FOR_APPROVAL:
            ...
            target = f"run {approval.tool_name} for {approval.target}" if approval else "perform the pending action"
            return ResponseFact(
        ...
        if state in {TaskState.EXECUTING, TaskState.VERIFYING, TaskState.RECOVERING}:
            running = next(
                (step for step in steps if step.status in {StepStatus.RUNNING, StepStatus.VERIFYING}),
                None,
            )
        ...
        if state is TaskState.FAILED and succeeded:
            failed = [f"{step.title} failed: {task.error or step.verification_detail or 'unverified'}" for step in failed_steps] or [
                f"Remaining work failed: {task.error or 'unverified'}"
            ]
            return ResponseFact(
                intent=ResponseIntent.PARTIAL_COMPLETION,
                succeeded_items=succeeded,
    ...
    def _accessibility_fact(
        ...
        application_names = {
            ...
            "me.adityalabs.omnimac.axtest": "OmniMac Accessibility Test App",
            "com.google.Chrome": "Chromium",
            "org.chromium.Chromium": "Chromium",
        }
        application_name = application_names.get(bundle_id, "the approved application") if isinstance(bundle_id, str) else "the approved application"
        ...
        evidence = " ".join(part for part in (task.error, ax_step.verification_detail) if isinstance(part, str)).lower()
        ...
        if any(token in evidence for token in ("capability", "unauthorized", "unsupported", "forbidden")):
            return fact(
    ...
    def _safe_fact(task: Task, original: ResponseFact) -> ResponseFact:
        steps = task.plan.steps if task.plan else []
        succeeded = [f"Step {step.index + 1} verified" for step in steps if step.status is StepStatus.SUCCEEDED]
        failed = [f"Step {step.index + 1} failed" for step in steps if step.status is StepStatus.FAILED]
