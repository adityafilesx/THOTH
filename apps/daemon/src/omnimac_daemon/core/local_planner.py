...
from omnimac_daemon.core.claude_planner import PLAN_SCHEMA, build_system_prompt
from omnimac_daemon.core.planner import PlannerAdapter
from omnimac_daemon.schemas import ExecutionPlan, RiskLevel, VerificationStrategy
...
class PlanValidator:
    ...
    def validate(self, raw: dict[str, Any], task_id: str) -> ExecutionPlan:
        ...
        normalized = {
            ...
            "steps": [{**{k: v for k, v in step.items() if k != "index"}, "index": i} for i, step in enumerate(raw_steps) if isinstance(step, dict)],
        }
        try:
        ...
        for step in plan.steps:
            ...
            if step.declared_risk.rank < default.rank:
                raise PlanRejected(
                    ...
                    f"{step.tool_name}: declared {step.declared_risk.value} below default {default.value}",
                )
            ...
            has_verifier = tool.verification is not VerificationStrategy.NONE_READONLY or bool(step.verification_checks)
            if effectful and not has_verifier:
                raise PlanRejected(
                    PlanRejection.MISSING_VERIFIER,
                    f"{step.tool_name}: effectful step ({step.declared_risk.value}) with no verifier",
                )

            if step.tool_name in _APP_ARG_TOOLS and self._apps is not None:
                app = step.arguments.get("app")
                if app not in self._apps:
                    raise PlanRejected(PlanRejection.UNSUPPORTED_APP, f"unsupported application {app!r}")
...
def plan_with_fallback(
    ...
    try:
        ...
        if skill:
            return LocalPlanResult(tier="skill", skill=skill, message=f"local planning failed: {exc}")
        return LocalPlanResult(
            tier="clarify",
            message=("I could not build a safe plan for that locally and no installed skill matches. Could you rephrase or name a skill?"),
        )
