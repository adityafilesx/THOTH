...
from omnimac_daemon.schemas import PolicyDecision, RiskLevel
...
class PolicyEngine:
    def evaluate(
        ...
        if effective is not declared_risk:
            reasons.append(f"declared risk {declared_risk.value} raised to tool default {tool_default_risk.value} (no downgrade)")
