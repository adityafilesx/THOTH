...
class OperationalDialogueStore:
    ...
    def resolve_follow_up(
        ...
        *,
        ...
        if state.pending_approval_id and _VAGUE_APPROVAL.fullmatch(normalized):
            raise ApprovalFollowUpRejected("dialogue cannot approve; use the invocation-bound approval endpoint")
    ...
    def _resolve_artifact(self, state: DialogueState, now: datetime) -> DialogueResolution:
        candidates = [
            artifact
            for artifact in state.referenced_artifacts
            if artifact.authoritative and artifact.created_at <= now and artifact.task_id == state.active_task_id and Path(artifact.path).is_file()
        ]
        if not candidates:
