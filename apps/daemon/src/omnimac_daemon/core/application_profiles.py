...
class ApplicationProfile(BaseModel):
    ...
    def _validate_capabilities(self) -> ApplicationProfile:
        ...
        if unknown_verifiers:
            raise ValueError(f"verifier mapping references undeclared capabilities: {unknown_verifiers}")
        ...
        missing_rules = {
            capability for capability in groups[0] | groups[1] if capability.startswith("ax_") and capability not in self.ax_capability_rules
        }
        if missing_rules:
...
class ApplicationProfileRegistry:
    def __init__(self, profiles: list[ApplicationProfile]) -> None:
        ...
        for profile in profiles:
            if profile.bundle_id in self._profiles:
                raise DuplicateApplicationProfile(f"duplicate application profile for {profile.bundle_id!r}")
        ...
        *,
        ...
        if capability in profile.experimental_capabilities:
            if not allow_experimental:
                raise CapabilityUnavailable(f"{capability!r} is experimental for {profile.display_name}")
            return CapabilityAuthorization(
                bundle_id=bundle_id,
                capability=capability,
    ...
    def authorize_ax(
        ...
        *,
        ...
        if rule.tool_name != tool_name:
            raise CapabilityTargetForbidden(f"{capability!r} does not authorize {tool_name!r} for {profile.display_name}")
        allowed_identifiers = rule.allowed_verifier_identifiers if verification_target else rule.allowed_identifiers
    ...
    def ax_rule(self, bundle_id: str, capability: str) -> AXCapabilityRule:
        ...
        if rule is None:
            raise CapabilityTargetForbidden(f"{capability!r} has no AX rule for {profile.display_name}")
        ...
        for profile in self._profiles.values():
            for rule in profile.ax_capability_rules.values():
                ...
                if rule.default_risk is not default_risk:
                    raise CapabilityTargetForbidden(f"{tool_name!r} risk conflicts with {profile.display_name} profile")
                if rule.focus_policy is not focus_policy:
                    raise CapabilityTargetForbidden(f"{tool_name!r} focus conflicts with {profile.display_name} profile")
            ...
            ax_rules={
                ...
                **{
                    capability: AXCapabilityRule(
                        ...
                        allowed_verifier_roles=(("AXTextArea",) if capability == "ax_set_value" else ()),
                        allowed_verifiers=(AXVerificationExpectation.VALUE_EQUALS,),
                        default_risk=(RiskLevel.R1 if capability == "ax_set_value" else RiskLevel.R0),
                        focus_policy=(mutation_focus if capability == "ax_set_value" else read_focus),
                    )
