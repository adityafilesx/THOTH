...
from omnimac_daemon.inference.base import InferenceRequest, InferenceResult

# Only these intents may be model-summarized; everything else is
# deterministic (safety-sensitive wording must not drift).
_MODEL_SUMMARIZABLE = frozenset({ResponseIntent.VERIFIED_COMPLETION, ResponseIntent.PARTIAL_COMPLETION})
...
class FactualConsistencyValidator:
    ...
    def validate(self, candidate: str, fact: ResponseFact, max_chars: int = 240) -> None:
        if len(candidate) > max_chars:
            raise FactualConsistencyError(f"summary is {len(candidate)} chars, over the {max_chars} limit")
        try:
        ...
        if _EXECUTION_OR_POLICY_DIRECTIVE.search(candidate):
            raise FactualConsistencyError("model summary contains an execution, approval, or policy directive")
        ...
        corpus = " ".join([fact.summary, *fact.succeeded_items, *fact.failed_items, fact.failure_reason or ""])
...
def _build_prompt(fact: ResponseFact, mode: ResponseMode, max_chars: int) -> InferenceRequest:
    ...
    system = (
        "You are Jarvis, a highly capable, concise, and professional voice-first AI assistant. "
        "Speak directly to the user as 'sir'. Rephrase the VERIFIED facts below into one plain, "
        "conversational sentence or two. Rules: state only what is in the facts; invent no numbers, "
        "names, paths, or claims; no filler ('Certainly', 'As an AI'); no emotion; do not claim "
        "anything not listed. "
