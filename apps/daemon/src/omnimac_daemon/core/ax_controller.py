...
from omnimac_daemon.core.ax_resolver import AXResolutionResult, AXResolver
...
from omnimac_daemon.macos.semantic_ax import RealSemanticAXAdapter, SemanticAXAdapter
...
class AXController:
    ...
    def set_value(
        ...
        *,
        ...
        return self._action_result(element, "AX set_value completed; independent verification pending")
    ...
    def perform_action(
        ...
        *,
        ...
        if action_name not in element.supported_actions:
            raise RuntimeError(f"AX action {action_name!r} is not supported by the resolved element")
        self._permissions.require_granted(now=self.now())
        self._raise_if_cancelled(cancelled)
    ...
    def select_option(
        ...
        *,
        ...
        return self._action_result(element, "AX option selection completed; independent verification pending")
    ...
    def verify(
        ...
        from omnimac_daemon.core.ax_verifiers import AXVerifierDispatcher
    ...
    def _require_element(
        ...
        *,
        ...
        observed = result.element.value_metadata.value if result.element.value_metadata is not None else None
