...
from omnimac_daemon.security.paths import expand_and_resolve, is_denied_path
...
class WorkspaceMatcher:
    ...
    def match(self, evidence: WorkspaceEvidence, now: datetime) -> WorkspaceMatch | None:
        ...
        for profile in self._profiles:
            ...
            if not profile.approved or (profile.expires_at is not None and profile.expires_at <= now) or not root.is_dir():
                continue

            authority: list[str] = []
            if evidence.task_workspace_id and _fold(evidence.task_workspace_id) == _fold(profile.workspace_id):
            ...
            hints: list[str] = []
            if evidence.active_bundle_id and _fold(evidence.active_bundle_id) in {_fold(bundle_id) for bundle_id in profile.app_bundle_ids}:
