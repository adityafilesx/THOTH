...
class LocalAIRuntimeManager:
    ...
    async def handle_memory_pressure(self, *, required_bytes: int) -> list[RuntimeComponent]:
        ...
        if loaded + required_bytes <= self._memory_limit:
            return []
        candidates = sorted(
            ((component, entry) for component, entry in self._entries.items() if entry.state is RuntimeState.IDLE_CACHED),
    ...
    async def set_battery_saver(self, enabled: bool) -> None:
        ...
        if enabled:
            candidates = [component for component, entry in self._entries.items() if entry.state is RuntimeState.IDLE_CACHED]
    ...
    def _estimated_loaded_bytes(self) -> int:
        ...
        return sum(entry.registration.memory_estimate_bytes for entry in self._entries.values() if entry.state not in unloaded)
