...
class BrowserSession(Protocol):
    ...
    async def submit(self, submission_id: str, timeout_s: float, expected_action_url: str | None = None) -> PageState: ...
...
class MockBrowserSession:
    ...
    def _form_fields(self, form_selector: str) -> dict[str, str]:
        page = self._page()
        return {el.name or sel: el.value or "" for sel, el in page.elements.items() if el.form == form_selector}
    ...
    async def submit(self, submission_id: str, timeout_s: float, expected_action_url: str | None = None) -> PageState:
        ...
        current = self._form_fields(prepared_meta.form_selector) if prepared_meta is not None else {}
...
class PlaywrightSession:
    ...
    async def _snapshot(self) -> PageState:
        ...
        return PageState(url=page.url, title=title, text=text[: 64 * 1024], selectors=tuple(selectors))
    ...
    async def _capture_fields(self, form_selector: str) -> tuple[str, dict[str, str]]:
        ...
        fields = await page.eval_on_selector(
            form_selector,
            "f => Object.fromEntries([...f.elements].filter(e => e.name).map(e => [e.name, e.value]))",
        )
