...
class ToolRegistry:
    ...
    def register(self, tool: ToolDefinition[Any, Any]) -> None:
        ...
        if not isinstance(getattr(tool, "focus_policy", None), FocusPolicy):
            raise InvalidToolDefinitionError(f"tool '{tool.name}' has no valid authoritative focus_policy")
    ...
    async def execute(self, invocation: ToolInvocation, allowed_scope: ResourceScope | None = None) -> ToolResult:
