...
def _view(state: PageState) -> PageView:
    return PageView(url=state.url, title=state.title, text=state.text, selectors=list(state.selectors))
...
async def _require_on_page(session: BrowserSession, current_url: str) -> None:
    ...
    if _host(state.url) != _host(current_url):
        raise ValueError(f"current_url host {_host(current_url)!r} does not match the session's actual page host {_host(state.url)!r}")
...
class BrowserFind(_SessionTool, ToolDefinition[FindIn, FindOut]):
    ...
    async def run(self, args: FindIn, dry_run: bool) -> FindOut:
        ...
        return FindOut(matches=[FoundElement(selector=e.selector, tag=e.tag, text=e.text, value=e.value) for e in found])
...
class BrowserDownload(_SessionTool, ToolDefinition[DownloadIn, DownloadOut]):
    ...
    async def run(self, args: DownloadIn, dry_run: bool) -> DownloadOut:
        ...
        return DownloadOut(path=await self._session.download(args.url, args.dest_dir, self.timeout_s))
...
class BrowserPrepareSubmission(_SessionTool, ToolDefinition[PrepareIn, PrepareOut]):
    ...
    description = "Capture exactly what a form WOULD submit (action URL + field map) without submitting; returns a single-use submission id."
    focus_policy = FocusPolicy.DO_NOT_STEAL_FOCUS
...
class BrowserSubmit(_SessionTool, ToolDefinition[SubmitIn, PageView]):
    name = "browser_submit"
    description = "Submit a PREVIOUSLY PREPARED form (single-use id). External side effect — requires explicit approval."
    focus_policy = FocusPolicy.DO_NOT_STEAL_FOCUS
    ...
    async def run(self, args: SubmitIn, dry_run: bool) -> PageView:
        if dry_run:
            return _view(await self._session.state())
        return _view(await self._session.submit(args.submission_id, self.timeout_s, expected_action_url=args.action_url))
