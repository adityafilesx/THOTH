...
from omnimac_daemon.macos.ax import AXAdapter, default_ax_adapter
...
class AXInspectApplication(ToolDefinition[AXInspectIn, AXInspectOut]):
    ...
    async def run(self, args: AXInspectIn, dry_run: bool) -> AXInspectOut:
        return AXInspectOut(app=args.app, elements=[_element(e) for e in self._ax.list_elements(args.app)])
...
class AXPerformAction(ToolDefinition[AXActionIn, AXActionOut]):
    ...
    async def run(self, args: AXActionIn, dry_run: bool) -> AXActionOut:
        ...
        if not ok:
            raise RuntimeError(f"could not perform {args.action} on {args.role}/{args.label} in {args.app}")
