...
from omnimac_daemon.schemas.enums import FocusPolicy, Provenance
...
class AXWindowSnapshot(_AXModel):
    ...
    def _element_count_matches(self) -> AXWindowSnapshot:
        ...
        if any(element.application_bundle_id != self.application_bundle_id for element in self.elements):
...
class AXVerificationRequest(_AXModel):
    ...
    def _target_matches_expectation(self) -> AXVerificationRequest:
        if self.target is not None and self.target.application_bundle_id != self.application_bundle_id:
        ...
        if self.expectation is AXVerificationExpectation.WINDOW_EXISTS and not self.window_identifier:
            raise ValueError("window verification requires a window identifier")
        if self.expectation is AXVerificationExpectation.VALUE_EQUALS and self.expected_value is None:
