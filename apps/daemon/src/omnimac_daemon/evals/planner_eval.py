...
from omnimac_daemon.schemas import ExecutionPlan, RiskLevel
    ...
    if case.expect.max_risk is not None:
        for risk in risks:
            if not risk <= case.expect.max_risk:
                failures.append(f"declared risk {risk.value} exceeds ceiling {case.expect.max_risk.value}")
...
def run_planner_evals(planner: PlannerAdapter, cases: list[EvalCase], planner_name: str) -> EvalReport:
    results: list[CaseResult] = []
    for case in cases:
        try:
...
def render_report_markdown(report: EvalReport) -> str:
    ...
    for r in report.results:
        ...
        lines.append(f"| {r.case_name} | {'PASS' if r.passed else 'FAIL'} | {plan_view or '—'} | {fail_view} |")
    lines.append("")
    return "\n".join(lines)
...
LIVE_CASES: list[EvalCase] = [
    ...
    EvalCase(
        ...
        expect=EvalExpectation(allowed_tools=["git_status", *_FS_READ], max_steps=3, max_risk=RiskLevel.R0),
    ),
    EvalCase(
    ...
    EvalCase(
        ...
        expect=EvalExpectation(allowed_tools=["shell_run", *_FS_READ], max_steps=2, max_risk=RiskLevel.R2),
    ),
]
