...
from omnimac_daemon.core.verifiers import VerifierContext, evaluate_check
...
def render_capstone_report(results: list[CapstoneResult], planner: str) -> str:
    ...
    for r in results:
        checks = ", ".join(f"{c.kind}:{'ok' if c.passed else 'FAIL'}" for c in r.check_results) or "—"
        lines.append(f"| {r.capstone} | {r.task_state} | {r.approvals_granted} | {'YES' if r.final_state_verified else 'no'} | {checks} |")
    lines.append("")
    for r in results:
        if r.detail:
...
CAPSTONES: list[CapstoneWorkflow] = [
    CapstoneWorkflow(
        ...
        final_checks=[
            VerificationCheck(kind=VerifierKind.FILE_EXISTS, params={"path": f"{WORKSPACE_PLACEHOLDER}/idea.txt"}),
            VerificationCheck(
    ...
    CapstoneWorkflow(
        ...
        final_checks=[
            ...
            VerificationCheck(kind=VerifierKind.FILE_EXISTS, params={"path": f"{WORKSPACE_PLACEHOLDER}/README.md"}),
        ],
    ),
    CapstoneWorkflow(
        name="research-and-save",
        goal=f"Read https://example.com and save research notes to {WORKSPACE_PLACEHOLDER}/research.md",
        ...
        reference_steps=[
            ...
            PlanStep(
                ...
                arguments={
                    ...
                    "content": "# Research: example.com\n\nCaptured by OmniMac capstone run.",
                },
                declared_risk=RiskLevel.R1,
            ),
        ],
    ...
    CapstoneWorkflow(
        ...
        final_checks=[
            VerificationCheck(kind=VerifierKind.GIT_STATE, params={"repo": WORKSPACE_PLACEHOLDER, "clean": False}),
            VerificationCheck(
