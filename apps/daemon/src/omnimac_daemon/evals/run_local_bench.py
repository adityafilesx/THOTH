"""CLI: benchmark local model candidates and write docs/LOCAL_MODEL_EVALUATION.md.
    ...
    uv run --project apps/daemon python -m omnimac_daemon.evals.run_local_bench \
...
from omnimac_daemon.inference import DeterministicInferenceProvider, LlamaCppInferenceProvider
...
async def _run(models: list[str], iterations: int) -> list[ModelBenchmark]:
    ...
    for model in models:
        ...
        bench = await benchmark_provider(provider, BENCH_CASES, risk_floor, iterations=iterations, model_id=model)
