...
async def create_task(body: CreateTaskBody, request: Request) -> dict[str, Any]:
    ...
    return await build_task_payload(request, settled)


@router.get("/api/tasks")
async def list_tasks(request: Request) -> list[dict[str, Any]]:
    return [await build_task_payload(request, task) for task in _orch(request).list_tasks()]
...
async def get_task(task_id: str, request: Request) -> dict[str, Any]:
    ...
    return await build_task_payload(request, task)
...
async def decide_approval(request_id: str, body: DecisionBody, request: Request) -> dict[str, Any]:
    try:
        task = await _orch(request).decide_approval(request_id, body.approved, body.modified_arguments)
