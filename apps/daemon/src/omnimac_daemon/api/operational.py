...
from omnimac_daemon.core.application_profiles import ApplicationProfileRegistry
...
from omnimac_daemon.core.persona_summary import PersonaSummaryComposer
from omnimac_daemon.core.runtime_status import LocalRuntimeMonitor
from omnimac_daemon.core.task_presentation import TaskPresentationComposer
from omnimac_daemon.inference.base import InferenceProvider
from omnimac_daemon.macos.ax_permission import AXPermissionService
from omnimac_daemon.schemas import Task
...
def _pending_for(request: Request, task_id: str) -> list[Any]:
    return [approval for approval in _orch(request).pending_approvals() if approval.task_id == task_id]
    ...
    if task.plan:
        for step in task.plan.steps:
            if step.verification_passed is True:
                ...
                if isinstance(path, str) and Path(path).is_file() and not any(artifact.artifact_id == step.id for artifact in artifacts):
                    artifacts.append(
                        ArtifactReference(
                            artifact_id=step.id,
                            task_id=task.id,
...
async def resolve_dialogue(task_id: str, body: DialogueResolveBody, request: Request) -> dict[str, Any]:
