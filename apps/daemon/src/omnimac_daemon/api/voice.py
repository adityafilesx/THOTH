...
from omnimac_daemon.api.operational import build_task_payload, refresh_dialogue
from omnimac_daemon.core.command_dispatch import CommandDispatcher, CommandDispatchResult
...
from omnimac_daemon.core.local_runtime import RuntimeUnavailable
...
from omnimac_daemon.schemas import TaskSource
from omnimac_daemon.storage.permissions import PermissionStore
from omnimac_daemon.voice.contracts import AudioCaptureMode
from omnimac_daemon.voice.service import VoiceCommandService, VoiceSessionRegistry
from omnimac_daemon.voice.session import TranscriptCorrectionExpired
from omnimac_daemon.voice.stop import GlobalStopAuthority
from omnimac_daemon.voice.stt import STTAdapter, STTUnavailableError
from omnimac_daemon.voice.tts import SpeechSynthesisService
from omnimac_daemon.voice.wake_word import WakeWordEngine
...
async def _submit_recent_follow_up(
    ...
    if resolution.intent in {
        ...
        if resolution.intent is DialogueIntent.READ_BACK:
            ...
            spoken = PersonaResponseComposer().compose(ResponseFact(intent=ResponseIntent.ACKNOWLEDGEMENT)).spoken
    ...
    if dispatched.task is None:
        response = dispatched.response or PersonaResponseComposer().compose(ResponseFact(intent=ResponseIntent.NEEDS_CLARIFICATION))
...
async def submit_voice_session(session_id: str, request: Request) -> dict[str, Any]:
    try:
        ...
        raise _voice_error(exc) from exc
    if result.stopped:
        response = result.response or PersonaResponseComposer().compose(ResponseFact(intent=ResponseIntent.INTERRUPTED))
        await _speak_safely(request, response.spoken)
        return {
            "stopped": True,
            "task": None,
...
@router.websocket("/api/voice/wakeword/stream")
