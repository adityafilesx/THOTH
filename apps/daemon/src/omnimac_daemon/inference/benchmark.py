...
def detect_hardware() -> HardwareInfo:
    def sysctl(key: str) -> str:
        try:
            return subprocess.run(["sysctl", "-n", key], capture_output=True, text=True, timeout=5, check=True).stdout.strip()
...
def score_plan(plan: dict[str, Any], case: BenchCase, risk_floor: dict[str, RiskLevel]) -> PlanScore:
    steps = _steps(plan)
    schema_valid = (
        isinstance(plan.get("summary"), str)
        and bool(steps)
        and all(isinstance(s.get("tool_name"), str) and s.get("declared_risk") in _VALID_RISKS for s in steps)
    )
    if not schema_valid:
        return PlanScore(
            schema_valid=False,
    ...
    tool_selection_ok = all(t in allowed for t in tools) and all(r in tools for r in case.required_tools)
...
def render_benchmark_markdown(hardware: HardwareInfo, results: list[ModelBenchmark], selected: str) -> str:
    ...
    lines = [
        ...
        "",
        f"**Selected default model: `{selected}`** (chosen by measured schema-valid % first, then latency within the memory budget).",
        "",
