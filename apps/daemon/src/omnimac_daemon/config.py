...
class Settings(BaseSettings):
    """Daemon configuration. Values come from OmniMac_* env vars / .env.

    Secrets never belong here: credentials live in the macOS Keychain.
    """

    model_config = SettingsConfigDict(env_prefix="OmniMac_", env_file=".env", extra="ignore")

    host: str = "127.0.0.1"
    port: int = 7710
    db_path: Path = Path("./data/omnimac.db")
    ...
    voice_correction_window_seconds: float = 15.0
