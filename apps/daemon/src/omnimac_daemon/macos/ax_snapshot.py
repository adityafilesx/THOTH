...
from omnimac_daemon.core.foreground import ForegroundRedactor
...
def build_window_snapshot(
    *,
    ...
    title, title_truncated = bounded_text(title, max_bytes=budget.max_string_bytes) if title is not None else (None, False)
...
def _value_metadata(node: AXObservedNode, limits: AXSnapshotLimits) -> AXValueMetadata | None:
    if node.value is None:
        return None
    sensitive_description = " ".join(part.lower() for part in (node.identifier, node.label, node.description) if part)
    sensitive = node.role == "AXSecureTextField" or any(
        marker in sensitive_description for marker in ("password", "passcode", "one-time", "verification code", "otp", "token")
    )
