...
from omnimac_daemon.schemas.ax import AXPermissionStatus
...
class AXAdapter(Protocol):
    ...
    def wait_for_element(self, app_name: str, role: str, label: str, timeout_s: float) -> AXElementInfo | None: ...
        ...
        for i, element in enumerate(elements):
            if element.role == role and element.label == label:
                if not element.enabled:
                    return False
                elements[i] = AXElementInfo(role=element.role, label=element.label, value=value, enabled=element.enabled)
                return True
        return False
