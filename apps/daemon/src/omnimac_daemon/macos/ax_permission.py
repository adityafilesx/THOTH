...
from omnimac_daemon.schemas.ax import AXPermissionState, AXPermissionStatus
...
class AXPermissionError(RuntimeError):
    ...
    def __init__(self, snapshot: AXPermissionSnapshot | str) -> None:
        ...
        if self.snapshot is None:
            ...
            super().__init__(f"Accessibility permission {self.snapshot.status.value}: {self.snapshot.detail}")
...
class AXPermissionService:
    ...
    def open_settings(self, *, user_requested: bool) -> bool:
        if not user_requested:
            raise SettingsOpenNotAuthorized("opening Accessibility settings requires an explicit user request")
