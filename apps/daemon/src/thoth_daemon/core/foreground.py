...
Privacy-limited foreground awareness. THOTH snapshots the current
...
class ForegroundContext(BaseModel):
    ...
    THOTH never captures the screen."""
...
class ForegroundContextBroker:
    ...
    def capture(self, reason: str, task_id: str | None, now: datetime) -> ForegroundContext:
        ...
        selected = (
            [self._redactor.redact_path(p) for p in self._selection()] if self._selection else []
        )
