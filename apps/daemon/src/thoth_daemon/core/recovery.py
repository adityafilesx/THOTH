    ...
    def on_step_failure(
        ...
        if len(episodes) > self.max_recovery_depth:
            return self._escalate(
                task_id,
                step_id,
                f"recovery depth exhausted ({self.max_recovery_depth} consecutive "
                "failing episodes); user intervention required",
            )
        ...
        if replans_done < self.max_replans_per_task:
            ...
            return RecoveryDecision(
                ...
                reason=f"retry budget exhausted; replanning ({replans_done + 1}"
                f"/{self.max_replans_per_task})",
            )

        return self._escalate(
            task_id,
            step_id,
            f"retry and replan budgets exhausted "
            f"({self.max_retries_per_step}/step, {self.max_replans_per_task} replans); "
