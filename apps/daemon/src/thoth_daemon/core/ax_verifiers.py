...
class AXElementValueVerifier(_ElementVerifier):
    ...
    def verify(self, request: AXVerificationRequest, capability: str) -> AXVerificationResult:
        ...
        passed = (
            ...
            and metadata.value == request.expected_value
        )
        return self._result(
            request,
            passed,
...
class AXElementEnabledVerifier(_ElementVerifier):
    ...
    def verify(self, request: AXVerificationRequest, capability: str) -> AXVerificationResult:
        ...
        return self._result(
            request, passed, f"enabled={element.enabled if element else None}", element
        )
...
class AXElementFocusedVerifier(_ElementVerifier):
    ...
    def verify(self, request: AXVerificationRequest, capability: str) -> AXVerificationResult:
        ...
        return self._result(
            request, passed, f"focused={element.focused if element else None}", element
        )
...
class AXElementSelectedVerifier(_ElementVerifier):
    ...
    def verify(self, request: AXVerificationRequest, capability: str) -> AXVerificationResult:
        ...
        return self._result(
            request, passed, f"selected={element.selected if element else None}", element
        )
...
class AXCompositeVerifier:
    ...
    def verify_all(
        ...
        results = [
            verifier.verify(request, capability)
            for verifier, request in zip(self._verifiers, requests, strict=True)
        ]
        ...
        return AXVerificationResult(
            ...
            detail="; ".join(
                f"{result.expectation.value}={'ok' if result.passed else 'fail'}: {result.detail}"
...
class AXVerifierDispatcher:
    ...
    def verify(
        ...
        if (
            request.expectation is AXVerificationExpectation.APPLICATION_FRONTMOST
            and self._app_control is not None
        ):
