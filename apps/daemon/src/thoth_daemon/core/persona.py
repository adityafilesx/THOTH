...
THOTH's voice: calm, precise, restrained, slightly formal, quiet during
...
_SUCCESS_WORDS = re.compile(
    r"\b(done|completed?|succeeded|success(fully)?|finished|all set)\b", re.IGNORECASE
)
    ...
    def check(self, text: str, fact: ResponseFact) -> None:
        ...
        if fact.intent is ResponseIntent.APPROVAL_REQUIRED:
            ...
            if _COMPLETION_CLAIM.search(text):
                raise ResponsePolicyViolation(
                    "completion language for a merely-proposed (approval-pending) action"
                )
        ...
        if fact.intent is ResponseIntent.VERIFIED_COMPLETION:
            if fact.verified is not True and _SUCCESS_WORDS.search(text):
                raise ResponsePolicyViolation(
                    "success language without a passing verification result"
                )
        elif fact.intent in _NO_SUCCESS_INTENTS and _SUCCESS_WORDS.search(text):
            raise ResponsePolicyViolation(
                f"success language is not permitted for a {fact.intent.value} response"
            )
...
class PersonaResponseComposer:
    ...
    def compose(
        self, fact: ResponseFact, mode: ResponseMode = ResponseMode.STANDARD
    ) -> PersonaResponse:
    ...
    def _template(self, fact: ResponseFact) -> str:
        ...
        if intent is ResponseIntent.APPROVAL_REQUIRED:
            ...
            return f"This will {target}. Nothing has been sent. Approve submission?"
        if intent is ResponseIntent.EXECUTION_PROGRESS:
            return fact.step_progress or fact.summary or "Working."
        ...
        if intent is ResponseIntent.PARTIAL_COMPLETION:
            ...
            return " ".join(p for p in (ok, bad) if p) or "The task partially completed."
        if intent is ResponseIntent.FAILED:
            reason = fact.failure_reason or "an error occurred"
            return f"The task failed: {reason}"
        if intent is ResponseIntent.POLICY_REFUSAL:
            reason = fact.failure_reason or "it is outside the approved scope."
            return f"I won't execute that. {reason}"
        if intent is ResponseIntent.NEEDS_CLARIFICATION:
            return fact.clarification_question or "Could you clarify what you'd like me to do?"
        if intent is ResponseIntent.INTERRUPTED:
            return "Stopped. No external action was taken."
        if intent is ResponseIntent.DEGRADED_MODE:
            return (
                "The local model is unavailable, so I'm limited to deterministic "
                "commands and installed skills."
            )
