...
from thoth_daemon.schemas import TaggedContent
...
class UntrustedContentError(Exception):
    def __init__(self, purpose: str, provenance: str) -> None:
        super().__init__(
            f"{provenance} content may not be used as {purpose}; "
            "only USER_TRUSTED or SYSTEM_TRUSTED provenance can cross this boundary"
        )
