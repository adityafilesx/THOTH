...
from thoth_daemon.schemas.ax import AXElementQuery, AXVerificationResult
...
class AXDiagnosticsStore:
            ...
            self._snapshot = self._snapshot.model_copy(
                update={
                    "verification_evidence": (
                        f"{result.expectation.value}="
                        f"{'verified' if result.passed else 'failed'}: {result.detail}"
                    ),
                    "updated_at": now,
                }
            )
    ...
    def record_permission_error(self, detail: str, *, now: datetime) -> None:
        with self._lock:
            self._snapshot = self._snapshot.model_copy(
                update={"permission_error": detail, "updated_at": now}
            )
