...
from thoth_daemon.core.verifiers.context import VerifierContext, VerifierOutcome
...
def _file_content(
    check: VerificationCheck, ctx: VerifierContext, _r: ToolResult
) -> VerifierOutcome:
    ...
    try:
        ...
        return VerifierOutcome(
            passed=False, detail=f"{path}: unreadable ({exc.__class__.__name__})"
        )
...
def _process_running(
    ...
    return VerifierOutcome(
        passed=running is should, detail=f"process {pattern!r} running={running}"
    )
...
def _port_listening(
    ...
    return VerifierOutcome(
        passed=listening is should, detail=f"{host}:{port} listening={listening}"
    )
...
def _http_health(check: VerificationCheck, ctx: VerifierContext, _r: ToolResult) -> VerifierOutcome:
    ...
    if "contains" in check.params and check.params["contains"] not in body:
        return VerifierOutcome(
            passed=False, detail=f"{url}: body missing {check.params['contains']!r}"
        )
...
def _browser_element(
    ...
    return VerifierOutcome(
        passed=present is present_want, detail=f"element {selector} present={present}"
    )


def _exit_code(
    check: VerificationCheck, _ctx: VerifierContext, result: ToolResult
) -> VerifierOutcome:
