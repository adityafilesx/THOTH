...
DISABLED BY DEFAULT. Constructed only when THOTH_ALLOW_CLOUD=1 AND an API
...
from thoth_daemon.inference.isolation import IsolationViolation
...
class AnthropicInferenceProvider(_MetricsMixin):
    def __init__(self, model: str = "claude-opus-4-8", isolation: bool = False) -> None:
        ...
        if os.environ.get("THOTH_ALLOW_CLOUD") != "1":
            raise InferenceUnavailableError(
                "cloud inference is disabled by default; set THOTH_ALLOW_CLOUD=1 to "
                "explicitly opt in (THOTH runs fully local without it)"
            )
        if not os.environ.get("ANTHROPIC_API_KEY"):
            raise InferenceUnavailableError("THOTH_ALLOW_CLOUD is set but ANTHROPIC_API_KEY is not")
        self._model = model
    ...
    async def generate(self, request: InferenceRequest) -> InferenceResult:  # pragma: no cover
        raise InferenceUnavailableError(
            "cloud generation is intentionally not wired into the local planner path"
        )
