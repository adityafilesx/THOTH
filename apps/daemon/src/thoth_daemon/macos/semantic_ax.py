...
class MockSemanticAXAdapter:
    ...
    def _replace_value(self, target: AXElementSnapshot, value: AXPrimitive) -> bool:
        ...
        if found:
            self._applications[target.application_bundle_id] = application.model_copy(
                update={"windows": tuple(windows)}
            )
        return found
...
def _sensitive_semantics(
    ...
    semantics = " ".join(
        part.lower() for part in (identifier, label, description) if part is not None
    )
