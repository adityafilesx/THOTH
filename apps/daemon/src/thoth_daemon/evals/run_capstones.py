"""CLI: run the five capstone workflows and write docs/CAPSTONE_REPORT.md.
    ...
    uv run --project apps/daemon python -m thoth_daemon.evals.run_capstones \
...
def _setup_workspace(root: Path) -> Path:
    ...
    git("init", "-b", "main")
    git("config", "user.email", "capstone@thoth.local")
    git("config", "user.name", "thoth-capstone")
...
async def _run_all(planner: str, skip: set[str]) -> list[CapstoneResult]:
    ...
    with tempfile.TemporaryDirectory(prefix="thoth-capstones-") as tmp:
        root = Path(tmp)
        for capstone in CAPSTONES:
            ...
            print(
                f"{capstone.name}: {result.task_state} "
                f"(approvals={result.approvals_granted}, "
                f"verified={result.final_state_verified}) {result.detail}"
            )
    return results
