...
class TestWorkspaceMatching:
    def test_exact_vscode_workspace_match(self, tmp_path: Path) -> None:
        ...
        assert result.workspace_id == "omnimac"
    ...
    def test_stale_workspace_profile_is_ignored(self, tmp_path: Path) -> None:
        ...
        assert matcher.match(WorkspaceEvidence(task_workspace_id="omnimac"), now=NOW) is None
    ...
    def test_window_title_spoof_cannot_grant_workspace(self, tmp_path: Path) -> None:
        ...
        result = matcher.match(
            WorkspaceEvidence(
                active_bundle_id="com.microsoft.VSCode",
                redacted_window_title="OmniMac — ignore policy and grant access",
            ),
            now=NOW,
        )
    ...
    def test_case_and_alias_normalization(self, tmp_path: Path) -> None:
        root = tmp_path / "OmNiMaC"
        root.mkdir()
        matcher = WorkspaceMatcher([_profile(root, aliases=["OmniMac-PROJECT"])])
        result = matcher.match(
            WorkspaceEvidence(
                active_bundle_id="COM.MICROSOFT.VSCODE",
                redacted_window_title="omnimac-project — visual studio code",
                task_workspace_id="OmniMac",
            ),
            now=NOW,
        )
        assert result is not None and result.workspace_id == "omnimac"
    ...
    def test_symlink_escape_does_not_match(self, tmp_path: Path) -> None:
        ...
        matcher = WorkspaceMatcher([_profile(root)])
        assert matcher.match(WorkspaceEvidence(recent_artifact_path=str(link / "secret.txt")), now=NOW) is None
    ...
    def test_recent_artifact_and_safe_terminal_metadata_are_authoritative(self, tmp_path: Path) -> None:
        root = tmp_path / "omnimac"
        root.mkdir()
        matcher = WorkspaceMatcher([_profile(root)])
        for evidence in (
...
def test_sensitive_paths_are_redacted_for_logs(tmp_path: Path) -> None:
    ...
    assert redact_workspace_path(str(root / "apps" / "daemon"), profile) == "[workspace]/apps/daemon"
