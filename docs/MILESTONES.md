...
- [x] **Phase 5.5 — command-path recovery:** typed, legacy voice, and session voice share the deterministic dispatcher; exact approval language never plans or approves; authoritative plans cannot be model-replaced; validator internals do not cross task/display boundaries; voice sessions are removed on dispatch exceptions; focus postconditions gate task success; bounded HTTP settlement no longer emits false 500 failures. Final gate: 995 daemon + 91 desktop tests.
