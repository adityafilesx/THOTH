...
- [~] **v1.0 release validation:** whisper.cpp v1.8.6 and tiny.en/base.en/small.en are installed and SHA-256 pinned; mismatch rejection, bundled-WAV local transcription, private cleanup, and 1,033 automated tests pass. No production STT model is selected because the real 30-command microphone/acoustic matrix is unrun. Exact-helper TCC is denied, Developer ID/notarization credentials are absent, and the 0.1.0 desktop-only DMG is not clean-install capable. Decision: `THOTH v1.0 RELEASE CANDIDATE`. See `docs/V1_RELEASE_REPORT.md`.
