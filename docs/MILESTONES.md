...
- [x] Implementation plan (`docs/superpowers/plans/2026-07-11-omnimac-phase-0-2.md`)
...
**OmniMac cannot control the computer until Phase 3 lands and is verified.**
...
- [x] **Phase 5.3 — application/workspace context:** six immutable versioned profiles; unknown/forbidden/experimental fail closed; authoritative path/task workspace matcher with title/bundle hints only; real Code + OmniMac path capstone verified.
...
- [~] **Phase 5.4 — Accessibility embodiment:** unlocked focus handoff, packaged unique-bundle test app, typed permission boundary, bounded untrusted snapshots, semantic resolver, ten narrow tools, independent verifiers, immutable profile rules, focus/cancellation ordering, desktop diagnostics, deterministic persona outcomes, resource ceilings, and adversarial coverage are complete. Real host packaging/identity and fail-closed permission evidence pass. Test-app/TextEdit UI capstones remain blocked because exact-helper TCC is denied. No AX capability has been promoted from experimental. See `docs/PHASE_5_4_CAPSTONE.md`.
...
- [x] **Phase 5.5 — command-path recovery:** typed, legacy voice, and session voice share the deterministic dispatcher; exact approval language never plans or approves; authoritative plans cannot be model-replaced; validator internals do not cross task/display boundaries; focus postconditions gate task success; bounded HTTP settlement no longer emits false 500 failures. Desktop failures use authoritative bounded presentation and voice routes distinguish skills/clarification/reflex/planner.
- [x] **Phase 5.5 — voice lifecycle hardening:** cancelled sessions are immediately discarded, global Stop removes active default-retention sessions, abandoned unsubmitted capture expires and is zeroed after a bounded local TTL, and daemon shutdown clears active capture state. Strict final gate: 996 daemon passed, 1 locked-screen skip, 95 desktop passed; database worker-thread warnings are fatal and clean.
- [~] **v1.0 release validation:** whisper.cpp v1.8.6 and base.en are bundled and manifest-pinned with the frozen daemon and exact helper; the app owns authenticated startup and normal/crash cleanup; the mounted DMG passes strict ad-hoc signature and asset verification. No production STT model is selected because the real 30-command microphone/acoustic matrix is unrun. Exact-helper TCC is unverified, Developer ID/notarization credentials are absent, Qwen/Chromium onboarding remains external, and clean-account installation is unrun. Decision: `OmniMac v1.0 RELEASE CANDIDATE`. See `docs/V1_RELEASE_REPORT.md`.
