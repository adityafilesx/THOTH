...
Finder, TextEdit, VS Code, Terminal, and Chromium expose only bounded inspection rules while real AX permission/evidence is absent. Terminal has no value-setting or action rule, so the restricted subprocess interface remains authoritative for shell execution. Chromium keeps `browser_dom` first and Accessibility second; AX does not duplicate DOM form automation. The fixture alone carries the narrow mutation rules needed for text, toggles, dropdown selection, Save/modal actions, delayed elements, and dynamic status verification.
