# THOTH Phase 0–2 Implementation Plan
...
**Goal:** Deliver THOTH Phase 0 (engineering docs + repo + Claude Code config), Phase 1 (Tauri/React shell + FastAPI daemon + WS + SQLite + logging + mocked UI), and Phase 2 (contracts, state machine, risk policy, approvals, tool registry with mocks, verification, recovery, audit, task API/WS) with tests.
...
Files: `apps/daemon/pyproject.toml`, `src/thoth_daemon/{main,app,config,logging_setup}.py`, `api/{health,ws}.py`, `events/bus.py`, `storage/{db,models}.py`, alembic init + first migration, tests.
